...
#
# The destination is NOT hard-coded: it comes from $PERMITGRAPH_VAULT. An earlier version
...
#
#   PERMITGRAPH_VAULT=/path/to/vault ./sync-to-drive.sh
set -euo pipefail
SRC="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)/"
DST="${PERMITGRAPH_VAULT:?set PERMITGRAPH_VAULT to the mirror destination}"
